...
const INTENT_SCHEMA = {
  ...
  properties: {
    ...
    capital: { type: "object", additionalProperties: false, properties: { currency: { type: "string", const: "USDC" }, amount: { type: "number", exclusiveMinimum: 0 } }, required: ["currency", "amount"] },
    ...
    constraints: { type: "array", items: { type: "object", additionalProperties: false, properties: { type: { type: "string", enum: ["MAX_WEIGHT", "MIN_WEIGHT", "MIN_CASH"] }, asset: { type: ["string", "null"], enum: ["NVDAc", "GOOGLc", "METAc", "AAPLc", "USDC", null] }, value: { type: "number", minimum: 0, maximum: 1 } }, required: ["type", "asset", "value"] } },
    targetAllocations: { type: "array", minItems: 1, maxItems: 5, items: { type: "object", additionalProperties: false, properties: { asset: { type: "string", enum: ["NVDAc", "GOOGLc", "METAc", "AAPLc", "USDC"] }, weight: { type: "number", exclusiveMinimum: 0, maximum: 1 } }, required: ["asset", "weight"] } },
    automation: { type: "object", additionalProperties: false, properties: { rebalance: { type: "string", enum: ["NONE", "WEEKLY", "MONTHLY", "QUARTERLY"] } }, required: ["rebalance"] },
  },
  required: ["intentVersion", "action", "capital", "themes", "risk", "exclusions", "constraints", "targetAllocations", "automation"],
} as const;

export class OpenRouterRequestError extends Error {
  constructor(public status: number, message: string, public code?: string | number) { super(message); this.name = "OpenRouterRequestError"; }
}
...
function parseStructuredContent(content: unknown): InvestmentIntent {
  ...
  try { return validateInvestmentIntent(JSON.parse(normalized)); }
  catch (error) { throw new OpenRouterRequestError(502, `OpenRouter returned invalid structured intent: ${error instanceof Error ? error.message : "invalid JSON"}`); }
}
...
export async function parseIntentWithOpenRouter(prompt: string, apiKey: string, model: string): Promise<OpenRouterIntentResult> {
  ...
  try {
    res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json", "HTTP-Referer": "https://stockos-ashen.vercel.app", "X-Title": "StockOS" },
      body: JSON.stringify({ models, temperature: 0, max_tokens: 1100, provider: { require_parameters: true }, messages: [{ role: "system", content: SYSTEM }, { role: "user", content: prompt }], response_format: { type: "json_schema", json_schema: { name: "stockos_investment_intent", strict: true, schema: INTENT_SCHEMA } } }),
    });
  } catch (error) {
...
const aliases: Array<{ asset: SupportedAsset; names: string[] }> = [
  { asset: "NVDAc", names: ["nvidia", "nvda", "nvdac"] }, { asset: "GOOGLc", names: ["google", "alphabet", "googl", "googlc"] },
  { asset: "METAc", names: ["meta", "facebook", "metac"] }, { asset: "AAPLc", names: ["apple", "aapl", "aaplc"] }, { asset: "USDC", names: ["cash", "usdc"] },
];
...
function allocationPercentageFor(prompt: string, names: string[]): number | null {
  ...
  for (const pattern of [new RegExp(`(?:${joined})[^,.;]{0,30}?(\\d+(?:\\.\\d+)?)\\s*%`, "i"), new RegExp(`(\\d+(?:\\.\\d+)?)\\s*%[^,.;]{0,30}?(?:${joined})`, "i")]) {
    const match = prompt.match(pattern); if (!match || CONSTRAINT_WORDS.test(match[0])) continue; return Number(match[1]) / 100;
  }
  return null;
}
function maxWeightFor(prompt: string, names: string[]): number | null {
  const joined = escapedNames(names);
  for (const pattern of [new RegExp(`(?:${joined})[^,.;]{0,20}?(?:max(?:imum)?|below|under|not more than|at most|cap(?:ped)?(?:\\s+at)?)\\s*(\\d+(?:\\.\\d+)?)\\s*%`, "i"), new RegExp(`(?:max(?:imum)?|not more than|at most|cap(?:ped)?(?:\\s+at)?)\\s*(\\d+(?:\\.\\d+)?)\\s*%[^,.;]{0,20}?(?:${joined})`, "i")]) {
    const match = prompt.match(pattern); if (match) return Number(match[1]) / 100;
  }
  return null;
}
function minCashFor(prompt: string): number | null {
  for (const pattern of [/(?:at least|minimum|min)\s+(\d+(?:\.\d+)?)\s*%\s*(?:cash|usdc)/i, /(?:cash|usdc)[^,.;]{0,20}?(?:at least|minimum|min)\s+(\d+(?:\.\d+)?)\s*%/i]) {
    const match = prompt.match(pattern); if (match) return Number(match[1]) / 100;
  }
  return null;
}
function distributeWithCaps(candidates: SupportedAsset[], investable: number, maxes: Map<SupportedAsset, number>) {
  ...
  if (!candidates.length || investable <= 0) return { weights, unallocated: Math.max(0, investable) };
  for (const asset of candidates) weights.set(asset, investable / candidates.length);
  let overflow = 0;
  for (const asset of candidates) { const cap = maxes.get(asset); const current = weights.get(asset)!; if (cap != null && current > cap) { overflow += current - cap; weights.set(asset, cap); } }
  let guard = 0;
  while (overflow > 1e-10 && guard++ < 20) {
    const open = candidates.filter(asset => (maxes.get(asset) ?? 1) - (weights.get(asset) ?? 0) > 1e-10); if (!open.length) break;
    const each = overflow / open.length; let consumed = 0;
    for (const asset of open) { const current = weights.get(asset) ?? 0; const capacity = (maxes.get(asset) ?? 1) - current; const add = Math.min(each, capacity); weights.set(asset, current + add); consumed += add; }
    if (consumed <= 1e-12) break; overflow -= consumed;
  }
...
export function demoIntent(prompt: string): InvestmentIntent {
  ...
  for (const entry of aliases.filter(entry => entry.asset !== "USDC")) { const value = maxWeightFor(prompt, entry.names); if (value != null) maxes.set(entry.asset, value); }
  ...
  for (const entry of aliases) { if (exclusions.includes(entry.asset)) continue; const weight = allocationPercentageFor(prompt, entry.names); if (weight != null && weight > 0) explicit.set(entry.asset, weight); }
  ...
  if (explicit.size) {
    ...
    const namedStocks = aliases.filter(entry => entry.asset !== "USDC" && !exclusions.includes(entry.asset) && mentioned(prompt, entry.names)).map(entry => entry.asset);
    ...
    const riskDefaultCash = risk === "conservative" ? 0.25 : risk === "moderate" ? 0.10 : 0.05;
    const defaultCash = selectionLanguage && namedStocks.length ? 0 : riskDefaultCash;
  ...
  return validateInvestmentIntent({ intentVersion: 2, action: /rebalance/i.test(prompt) ? "REBALANCE" : /update|change|modify/i.test(prompt) ? "UPDATE_PORTFOLIO" : "CREATE_PORTFOLIO", capital: { currency: "USDC", amount }, themes: [/(?:ai|artificial intelligence)/i.test(prompt) ? "artificial_intelligence" : "custom"], risk, exclusions, constraints, targetAllocations, automation: { rebalance: /weekly/i.test(prompt) ? "WEEKLY" : /monthly/i.test(prompt) ? "MONTHLY" : /quarterly/i.test(prompt) ? "QUARTERLY" : "NONE" } });
}
